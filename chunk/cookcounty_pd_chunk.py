"""
Cook County Public Defender website chunker.

Reads cookcounty-pd/cookcounty_pd_corpus.jsonl from the raw S3 bucket and
writes cookcounty-pd/cookcounty_pd_chunks.jsonl to the chunked S3 bucket.
Defaults to S3 I/O; use --local-only for development.

Corpus: 12 records — FAQ pages, resource directories, and informational pages.
Notable: the "free-and-low-cost-legal-assistance-resources" record (~28K chars)
is a directory of ~50 legal aid organizations; it needs careful splitting.

Chunking strategy:
  1. Collapse excess whitespace
  2. For the large legal-aid directory: split at organization-entry boundaries
     (detected by the repeating "Type of Legal Assistance" label that starts
     each entry's detail block)
  3. For all other records: paragraph-aware token accumulation
     (target 600 tokens, max 800, last-paragraph overlap)
  4. Merge micro-chunks (< MIN_CHUNK_TOKENS) into neighbors

Usage:
    python3 chunk/cookcounty_pd_chunk.py            # S3 -> S3
    python3 chunk/cookcounty_pd_chunk.py --local-only
"""
...
def get_config() -> dict:
    return {
        ...
        "raw_key":        "cookcounty-pd/cookcounty_pd_corpus.jsonl",
        "chunked_bucket": _require_env("CHUNKED_S3_BUCKET"),
        "chunked_key":    "cookcounty-pd/cookcounty_pd_chunks.jsonl",
...
# Record ID of the large legal-aid directory that needs org-boundary splitting
_LEGAL_AID_DIRECTORY_ID = "ccpd-resources-free-and-low-cost-legal-assistance-resources"
...
# Non-breaking space (common in HTML-extracted text)
...
# ---------------------------------------------------------------------------
# Organization-boundary splitting (for legal-aid directory)
#
# Each entry in the directory has the repeating pattern:
#   <org name + address block>
#   Type of Legal Assistance
#   <description>
#   To Request Legal Assistance / Eligibility Requirements / Tips / Hours / ...
#
# "Type of Legal Assistance" is the most reliable entry-start anchor.
# ---------------------------------------------------------------------------

_ORG_ENTRY_RE = re.compile(
    r"(?:^|\n)(?=Type of Legal Assistance\s*\n)",
    re.MULTILINE,
)


def split_directory_into_entries(text: str) -> list[str]:
    """Split the legal-aid directory at each org entry boundary."""
    parts = _ORG_ENTRY_RE.split(text)
...
def _make_enriched(rec: dict, text: str) -> str:
    header = f"Cook County Public Defender — {rec.get('page_title', '')}"
...
def chunk_record(rec: dict) -> list[CookCountyPDChunk]:
    ...
    # Large legal-aid directory: split at org-entry boundaries first, then
    # further split any entry that still exceeds MAX_TOKENS
    if rec_id == _LEGAL_AID_DIRECTORY_ID:
        entries = split_directory_into_entries(text)
        raw_chunks: list[str] = []
        for entry in entries:
            raw_chunks.extend(split_text(entry))
    else:
    ...
    for i, chunk_text in enumerate(filtered):
        result.append(CookCountyPDChunk(
...
def run(local_only: bool = False, limit: int = 0) -> None:
    log.info("=== Cook County PD chunking pipeline starting ===")
    cfg     = get_config()
    ...
    if local_only:
        write_local(all_chunks, LOCAL_OUTPUT_DIR / "cookcounty_pd_chunks.jsonl")
    else:
        write_s3(all_chunks, cfg["chunked_bucket"], cfg["chunked_key"], cfg["aws_region"])

    log.info("=== Cook County PD chunking pipeline complete ===")


def main() -> None:
    parser = argparse.ArgumentParser(description="Chunk Cook County PD JSONL → chunks JSONL")
